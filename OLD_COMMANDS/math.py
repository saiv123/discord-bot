...
class math(commands.Cog):
    ...
    async def wolfram(self, ctx: SlashContext, query: str = ""):
        ...
        embed.set_thumbnail(
            url="https://cdn.iconscout.com/icon/free/png-512/wolfram-alpha-2-569293.png"
        )
        ...
        for i in range(len(res)):
            ...
            for msg in splitLongStrings(res[i].text, chars=1024):
                name = (
                    f"Answer {i+1}:"
                    if opener and len(res) > 1
                    else str(chr(0xFFA0) * (i + 1))
                )
                embed.add_field(name=str(name), value=str(msg), inline=False)
                opener = False

        embed.set_footer(
            text=f"Answer Requested by: {ctx.author.name}",
