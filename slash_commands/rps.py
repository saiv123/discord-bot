...
class rps(commands.Cog):
    ...
    async def rps(self, ctx: SlashContext, *, level=1, guild_ids=[648012188685959169]):
        if level > RPS_HARD_CAP and not isOwner(ctx):
            msg = add_to_embed(
                "Level too high!",
                f"Sorry, but even though the code for it exists, why would you ever want to play rps-{level*2+1}???",
            )[0]
            msg.set_footer(
                text="RPS Played by: " + ctx.author.name, icon_url=ctx.author.avatar_url
            )
        ...
        try:
            ...
            embed = add_to_embed(
                f"{ctx.author.name}'s RPS", "Awww, don't leave me hangin'"
            )[0]
            embed.set_footer(
                text="RPS Played by: " + ctx.author.name, icon_url=ctx.author.avatar_url
            )
        ...
        if "rules" in mlo:
            ...
            distance(freeform, mlo) >= len(freeform) * 0.3
        ):  # If the most likely option is more than 30% wrong, hassle
            output = "No option recognized! Your choices are: " + "\n".join(
                ["rules"] + symbol_names[: level * 2 + 1]
            )
        else:
    ...
    async def rpsc(
        ...
        embed = add_to_embed(
            f"Your challenge to {user.name}", msg + "\nCheck your DMs!"
        )[0]
        embed.set_footer(
            text="Challenge sent by: " + ctx.author.name, icon_url=ctx.author.avatar_url
        )
        await ctx.send(embed=embed)
        ...
        async def get_response(_user, title="RPSC", timeout=10 * 60, opener=""):
            ...
            while i < 3:
                i += 1
                for msg in add_to_embed(
                    title,
                    f"{opener}Your choices are "
                    + ", ".join(symbol_names[: 2 * level + 1] + ["rules", "abort"]),
                ):
                ...
                try:
                    msg = await self.bot.wait_for(
                        "message", check=get_check(_user), timeout=timeout
                    )
                except:
                    await _user.send(
                        embed=add_to_embed(
                            title, f"Awww, {_user.name} don't leave me hangin'"
                        )[0]
                    )
                    return -1  # Abort challenge if you don't send an answer

                response = msg.content.lower().replace(" ", "_").replace("\n", "")
                choice = getClosestFromList(
                    ["abort", "rules"] + symbol_names, response.lower()
                )
                if distance(response, choice) >= len(response) * 0.3:
                    await _user.send(
                        embed=add_to_embed(choice, "No option recognized, try again")
                    )
        ...
        # Get your response
        your_choice = await get_response(
            ctx.author, title=f"Your challenge to {user.name}"
        )
        if your_choice == -1:
            await ctx.author.send(
                embed=add_to_embed(
                    f"Your challenge to {user.name}", "Challenge cancelled!"
                )[0]
            )
            await ctx.channel.send(
                embed=add_to_embed(
                    f"{ctx.author.name}'s challenge", "Challenge cancelled!"
                )[0]
            )
            return
        ...
        await ctx.author.send(
            embed=add_to_embed(
                f"Your challenge to {user.name}",
                f"You chose {symbol_names[your_choice]}",
            )[0]
        )
        ...
        enemy_choice = await get_response(
            user,
            title=f"{ctx.author.name}'s challenge",
            opener=f"{ctx.author.name} has challenged you to rock-paper-scissors-"
            + str(level * 2 + 1)
            if level > 1
            else "",
        )
        if enemy_choice == -1:
            ...
            await ctx.author.send(
                embed=add_to_embed(
                    f"Your challenge to {user.name}", "Challenge cancelled by opponent"
                )[0]
            )
            return
        ...
        await user.send(
            embed=add_to_embed(
                f"{ctx.author.name}'s challenge",
                f"You chose {symbol_names[enemy_choice]}",
            )[0]
        )
        ...
        if winner == 0:
            await ctx.author.send(
                embed=add_to_embed(
                    f"Your challenge to {user.name}",
                    msg.replace(ctx.author.name, "You") + "\nThe bout ended in a draw",
                )[0]
            )
            await user.send(
                embed=add_to_embed(
                    f"{ctx.author.name}'s challenge",
                    msg.replace(user.name, "You") + "\nThe bout ended in a draw",
                )[0]
            )
            msg += "\nThe bout ended in a draw"
        elif winner == 1:
