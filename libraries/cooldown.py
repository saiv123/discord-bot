...
with botDB.BotDB() as db:
    db.execute(
        """CREATE TABLE IF NOT EXISTS COOLDOWNS ([user] TEXT PRIMARY KEY NOT NULL, [lastused] REAL NOT NULL, [data] TEXT NOT NULL)"""
    )
...
class UserCooldownException(Exception):
    ...
    def get_msg(self, give_time_left: bool = True, give_command_law: bool = True):
        ...
        tim = f" {self.time_left_msg()}s left"
        tim = "" if "unknown" in tim.lower() or not give_time_left else tim
        law = f" Can be used {self.law_msg()}"
        law = "" if "unknown" in law.lower() or not give_command_law else law
...
def clean(
    days=30,
):  # If you don't use a command with a cooldown in 30 days, you get purged
    with botDB.BotDB() as db:
        cursor.execute(
            f"""DELETE FROM COOLDOWNS WHERE lastused < {time.time()-24*3600*days}"""
        )
...
def force_use_cmd(user: discord.User or str, command: str):
    ...
    if isinstance(user, discord.User):
        user = user.id
    user = str(user)
    conn = sqlite3.connect(botDB.DB_NAME)
    cursor = conn.cursor()
    ...
    if not isinstance(user_info, dict):
        raise Exception(f"Incorrect entry in COOLDOWNS table for user {user}")
    ...
    cursor.execute(
        ...
        (str(user), time.time(), str(user_info)),
    )
    conn.commit()
    cursor.close()
    conn.close()
...
def use_cmd(
    user: discord.User or str,
    command: str,
    cooldown: float,
    uses=1,
    use_first_use: bool = False,
):
    """
    ...
    if command in user_info:
        # Check for cooldown
        last_for_cooldown_purposes = (
            user_info[command]["first_use"]
            if use_first_use
            else user_info[command]["times_used"]
        )
        if (
            time.time() < cooldown + last_for_cooldown_purposes
            and user_info[command]["times_used"] >= uses
        ):
            print(f"Cooldown command {command} on cooldown")
            raise UserCooldownException(
                user,
                command,
                tl=cooldown - (time.time() - last_for_cooldown_purposes),
                mt=cooldown,
                x=uses,
            )
...
def has_cooldown(
    ...
    def wrapper(func):
        ...
        def wrap(*args, **kwargs):
            user = args[1].author
            if (isOwner(user) and owner_exempt) or (
                user.guild_permissions.administrator and admin_exempt
            ):
                ...
                use_cmd(
                    user, cmd_name, cooldown, uses=times, use_first_use=use_first_use
                )
