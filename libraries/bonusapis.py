...
def dumbTrumpQuote(tag: str = ""):
    ...
    if (
        "_embedded" in resp
        and "source" in resp["_embedded"]
        and len(resp["_embedded"]["source"]) > 0
        and "url" in resp["_embedded"]["source"][0]
    ):
...
def get_max_sim(list1, list2):
    ...
    best = max(
        (wordnet.wup_similarity(s1, s2) or 0, s1, s2)
...
def get_contradiction_score(phrase1: str, phrase2: str):
    raw_score = 1 / (
        abs(
            get_sentiment(phrase1, average=False)
            - get_sentiment(phrase2, average=False)
        )
        + abs(phrase1.count("not") - phrase2.count("not")) / 2.0
    )
    return (
        round(4 * math.log2(1 + raw_score)) / 4
    )  # Round to nearest 0.25 and use log2 to make spread smaller (might as well use a root)
...
def getColor(entry: str, code=""):
    ...
    resp = getReqJSON(
        "https://www.thecolorapi.com/id?format=json&" + str(code) + "=" + str(entry)
    )
    if str(resp).count("None") > 2 or "hex" not in resp:
        raise ValueError

    color_dict = {
        "url": "https://www.thecolorapi.com/id?format=html&"
        + str(code)
        + "="
        + str(entry)
    }
    if "hex" in resp:
        color_dict["hex"] = resp["hex"]["clean"]
    if "rgb" in resp:
        color_dict["rgb"] = [resp["rgb"]["r"], resp["rgb"]["g"], resp["rgb"]["b"]]
    if "hsl" in resp:
        color_dict["hsl"] = [resp["hsl"]["h"], resp["hsl"]["s"], resp["hsl"]["l"]]
    if "hsv" in resp:
        color_dict["hsv"] = [resp["hsv"]["h"], resp["hsv"]["s"], resp["hsv"]["v"]]
    if "XYZ" in resp:
        color_dict["xyz"] = [resp["XYZ"]["X"], resp["XYZ"]["Y"], resp["XYZ"]["Z"]]
    if "cmyk" in resp:
        color_dict["cmyk"] = [
            resp["cmyk"]["c"],
            resp["cmyk"]["m"],
    ...
    if "image" in resp and "named" in resp["image"]:
        color_dict["img"] = resp["image"]["named"]
    if "name" in resp:
        if "value" in resp["name"]:
            color_dict["name"] = resp["name"]["value"]
        if "distance" in resp["name"]:
            color_dict["distance"] = resp["name"]["distance"]
...
def colorDictToEmbed(color_dict, titled=True, named=True):
    ...
    if "hex" in color_dict:
        kwargs_dict["color"] = int(color_dict["hex"], 16)
    ...
    for val in vals_to_add:
        if val in color_dict:
            embed.add_field(
                name=val.upper(),
                value="({})".format(", ".join([str(i) for i in color_dict[val]])),
                inline=True,
            )

    # add image
    url_add = "&named=False" if not named else ""
    if "img" in color_dict:
        embed.set_image(url=color_dict["img"] + url_add)
