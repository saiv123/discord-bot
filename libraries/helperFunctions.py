...
def add_to_embed(
    ...
    try:
        ...
        pass
    try:
        dummy_embed.footer = embed.footer
    except:
        pass
    try:
    ...
    for i in range(len(message)):
        msg_txt = splitLongStrings(
            message[i],
            chars=chars,
            preferred_char="\n"
            if message[i].count("\n") >= len(message[i]) / (1.5 * chars)
            else " ",
        )
        if use_description:
            embeds[i].description = (
                f"{embeds[i].description}\n{msg_txt.pop(0)}"
                if len(embeds[i].description)
                else msg_txt.pop(0)
            )
...
def getEmbedsFromLibraryQuery(libraryPath, query):
    ...
    namedImg = (
        "Error",
        "https://www.prajwaldesai.com/wp-content/uploads/2014/01/error-code.jpeg",
    )
...
def gen_rps_matrix(size):
    row = [0] + [
        i % 2 + 1 for i in range(2 * size)
    ]  # baseline rps winner matrix 0 1 2 1 2 ...
    matrix = [row]
...
def format_matrix(matrix, symbol_names):
    ...
    for p1 in range(len(matrix)):
        for p2 in range(p1 + 1, len(matrix[p1])):
            ...
            lines.append(
                str(list_god(symbol_names, winner_symbol, "Nothing"))
                + " beats "
                + str(list_god(symbol_names, loser_symbol, "nothing"))
            )
    return lines
...
def HexToRgb(hex):
    ...
    )  # using tuple object to split the string to convert to rgb
    print(rgb)
    return rgb
